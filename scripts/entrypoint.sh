...
until MIGRATE_OUTPUT=$(POSTGRES_URL="$MIGRATE_URL" /usr/local/bin/auth-migrate 2>&1) || [ $RETRY_COUNT -eq $MAX_RETRIES ]; do
...
echo "=========================================="
POSTGRES_URL="$MIGRATE_URL" /usr/local/bin/auth-seed || echo "Seed completed with warnings (non-fatal)"

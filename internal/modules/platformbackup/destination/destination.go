...
// for the auth-api PLATFORM backup (the pg_dumpall DR backup). The local PVC copy
// produced by the platform backup scheduler is ALWAYS the primary, durable store; a
// configured remote destination (OneDrive / Google Drive / S3 / WebDAV / SFTP / SMB)
// is an additional, mirrored copy uploaded best-effort via rclone (see uploader.go).
//
// The platform backup is platform-scoped (no tenant), so a SINGLE platform-default
// destination is stored. It lives in the existing IntegrationConfig table under the
// reserved name "backup_destination" with tenant_id NULL (platform scope). The
// serialized Destination JSON is encrypted at rest with AES-256-GCM using a
// SECRET_KEY-derived key (see env.go / Cipher), so secret backend params (access
// keys, tokens, passwords) are never stored or logged in plaintext.
//
// This package is intentionally self-contained so it can be lifted into the other
// Go services that share the IntegrationConfig + SECRET_KEY pattern.
package destination
...
// and portable across services. auth-api passes a SECRET_KEY-derived implementation
// (see SecretKeyCipher) that provides PrimaryKey + CandidateKeys.
...
// ConfigName is the IntegrationConfig.name under which the (encrypted) platform
// backup destination JSON is stored at platform scope (tenant_id NULL).
const ConfigName = "backup_destination"
...
// (see the per-type SECRET/required key sets below); the uploader maps them onto
// rclone RCLONE_CONFIG_* env vars.
type Destination struct {
	...
	// under (e.g. "auth/backups", an S3 key prefix, or a Drive folder path).
...
// Store reads/writes the single encrypted platform Destination config in the
// IntegrationConfig table (name = ConfigName, tenant_id NULL). It is the single
// owner of the encrypt/decrypt + upsert logic, using AES-256-GCM (encrypt with the
// PRIMARY key; decrypt by trying each candidate key in priority order).
type Store struct {
	client *ent.Client
	keys   Cipher
	logger *zap.Logger
...
// NewStore constructs a Store over the ent client and a Cipher (the SECRET_KEY
// cipher). The Cipher provides the encrypt/decrypt key material.
...
// Resolve returns the effective platform destination: the stored, enabled
// platform-default row, else PVCOnly. A disabled stored destination resolves to
// PVCOnly so callers uniformly treat "no remote" the same way. Credentials are
// never logged.
func (s *Store) Resolve(ctx context.Context) Destination {
	if d, ok := s.load(ctx); ok && d.Enabled {
		return d
	}
	return PVCOnly()
}

// Get returns the stored platform destination WITHOUT the enabled-fallback, for
// the management endpoints. ok=false when no row exists.
func (s *Store) Get(ctx context.Context) (Destination, bool) {
	return s.load(ctx)
}

// load reads and decrypts the platform destination row (name = ConfigName,
// tenant_id NULL). Returns ok=false when absent or unreadable (never panics,
// never logs credential material).
func (s *Store) load(ctx context.Context) (Destination, bool) {
	row, err := s.client.IntegrationConfig.Query().
		Where(integrationconfig.Name(ConfigName), integrationconfig.TenantIDIsNil()).
		First(ctx)
	if err != nil {
	...
	d, err := s.decrypt(ctx, row.EncryptedCredentials)
	if err != nil {
...
// Store encrypts and upserts the platform destination (name = ConfigName,
// tenant_id NULL). The encrypted JSON lives in the Sensitive encrypted_credentials
// column so it is never echoed by generic listing endpoints.
func (s *Store) Store(ctx context.Context, d Destination) error {
	enc, err := s.encrypt(ctx, d)
	if err != nil {
		return fmt.Errorf("encrypt destination: %w", err)
	}

	existing, qErr := s.client.IntegrationConfig.Query().
		Where(integrationconfig.Name(ConfigName), integrationconfig.TenantIDIsNil()).
		First(ctx)
	if qErr == nil {
		_, uErr := existing.Update().
			SetEncryptedCredentials(enc).
			SetIsActive(d.Enabled).
			SetStatus(statusFor(d)).
			Save(ctx)
		return uErr
	}

	_, cErr := s.client.IntegrationConfig.Create().
		SetName(ConfigName).
		SetDisplayName("Platform Backup Destination").
		SetDescription("Encrypted pluggable backup destination (rclone) config for the platform pg_dumpall DR backup").
		SetEncryptedCredentials(enc).
		SetIsActive(d.Enabled).
		SetStatus(statusFor(d)).
		Save(ctx)
...
// statusFor maps a destination to the IntegrationConfig status enum.
func statusFor(d Destination) string {
	if d.IsRemote() {
		return "active"
	}
	return "inactive"

...
// Uploader mirrors a locally-written platform backup file to the configured remote
...
// Mirror copies localFilePath to the resolved platform remote destination under
...
// and are NEVER logged. Platform scope — no tenant.
func (u *Uploader) Mirror(ctx context.Context, localFilePath, objectName string) error {
	d := u.store.Resolve(ctx)
	if !d.IsRemote() {
	...
	if err := cmd.Run(); err != nil {
		...
		u.logger.Warn("platform backup remote mirror failed (PVC copy retained as fallback)",
	...
	u.logger.Info("platform backup mirrored to remote destination",
		zap.String("type", string(d.Type)),
		zap.String("object", objectName))
	return nil
}
